...
def num_islands2(m, n, positions)
  grid = m.times.collect { [0] * n }
  count = 0
  res = []
  positions.each_with_index do |(row, col), island_id|
    if !grid[row][col].zero?
      res.push(count)
      next
    end
    count += 1
    grid[row][col] = island_id + 1
    min_neighbour = min_neighbour(grid, row, col, m, n)
    if min_neighbour != grid[row][col]
      count -= 1
      grid[row][col] = min_neighbour
      [[row + 1, col], [row - 1, col], [row, col + 1], [row, col - 1]].each do |(irow, icol)|
        next if irow < 0 || icol < 0 || irow >= m || icol >= n || grid[irow][icol].zero?
        count -= 1 if grid[irow][icol] > min_neighbour
        flood_fill(grid, irow, icol, m, n, min_neighbour)
      end      
    end
    res.push(count)
  end
...
def min_neighbour(grid, row, col, m, n)
  min = grid[row][col]
  [[row + 1, col], [row - 1, col], [row, col + 1], [row, col - 1]].each do |(irow, icol)|
    next if irow < 0 || icol < 0 || irow >= m || icol >= n || grid[irow][icol].zero?
    min = grid[irow][icol] if min > grid[irow][icol]
  end
  min
end

def flood_fill(grid, row, col, m, n, val)
  return if row < 0 || row >= m || col < 0 || col >= n || grid[row][col].zero?
  return if grid[row][col] == val
  grid[row][col] = val
  flood_fill(grid, row + 1, col, m, n, val)
  flood_fill(grid, row - 1, col, m, n, val)
  flood_fill(grid, row, col + 1, m, n, val)
  flood_fill(grid, row, col - 1, m, n, val)
end

#p num_islands2 3, 3, [[0, 0], [0, 1], [1, 2], [2, 1], [2,2]]
p num_islands2 3, 3, [[0,1],[1,2],[2,1],[1,0],[0,2],[0,0],[1,1]]
