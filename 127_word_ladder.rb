...
def distance(word1, word2)
  dist, idx1, idx2 = 0, 0, 0
  while idx1 < word1.length && idx2 < word2.length
    dist += 1 if word1[idx1] != word2[idx2]
    idx1 += 1
    idx2 += 1
  end
  dist + (idx2 - idx1).abs
...
def create_graph(words)
  ...
  words.each do |word|
    ...
    words.each do |other_word|
      next if other_word == word || distance(word, other_word) != 1
      dict[word] << other_word
    end
  end
end
...
def ladder_length(begin_word, end_word, word_list)
  ...
  hash = create_dist_map(word_list, )
  visited = Set.new([begin_word])
  queue = [[begin_word, 1]]
  while !queue.empty?
    cur_word, len = queue.shift
    return len if cur_word == end_word
    word_list.each do |word|
      next if word == cur_word || distance(cur_word, word) != 1
      next if visited.include?(word)
      visited.add(word)
      queue.push([word, len + 1])
    end
  end
  0
end
...
def ladder_length(begin_word, end_word, word_list)
  ...
  while !queue.empty?
    cur_word, len = queue.shift
    return len if cur_word == end_word    
    word_list.select{|next_word| single_dist?(cur_word, next_word)}.each do |word|
...
def ladder_length(begin_word, end_word, word_list)
  ...
  while !reached.include?(end_word)
    ...
    reached.each do |word|
      for_each_combo(word) do |next_word|
        next unless word_list.include?(next_word)
        word_list.delete(next_word)
        next_level << next_word
      end
    end
    distance += 1
    return 0 if next_level.empty?
    reached = next_level
  end
  distance
end

def for_each_combo(word)
  word = word.chars
  res = []
